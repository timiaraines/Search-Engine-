...
def search(keyword):
    ...
    for row in range(0, len(data)):
        for col in range(0, len(data[row])):
            data_list = data[row][4]
        if word in data_list:
            metadata.append(data[row][:4])
...
def article_length(max_length, metadata):
    ...
    for row in range(0, len(metadata)):
        for col in range(0, len(metadata[row])):
            data_list = metadata[row][3]
        if max_length >= data_list:
            data.append(metadata[row][:4])
    return data
...
def favorite_author(favorite, metadata):
    ...
    for row in range(0, len(metadata)):
        for col in range(0, len(metadata[row])):
            data_list = metadata[row][1]
        if favorite in metadata[row][1]:
            return True
...
def multiple_keywords(keyword, metadata):
    ...
    for row in range(0, len(data)):
        for col in range(0, len(data[row])):
            data_list = data[row][4]
        if word in data_list:
            second_search.append(data[row][:4])
        final_search = first_search + second_search
    return final_search  
